...
const EditStoreModal = ({ isOpen, onClose, store, onStoreUpdated }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      addToast(err.response?.data?.message || 'Failed to update store', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Edit Store Details">
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1">Store Email</label>
        ...
        <div>
          <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1">Store Owner (Optional)</label>
